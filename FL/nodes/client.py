...
import sys
# ??
sys.path.append(r'C:\Users\btsma\agnostic_federated_learning') # maybe might work idk
...
class CreateDataset(Dataset):
    ...
    """
    def __init__(self, dataset, idxs, noise_variance=0.01):
        self.dataset = dataset
        self.idxs = [int(i) for i in idxs]
        self.noise_variance = noise_variance

    def add_noise(self, image):
        noise = torch.randn_like(image) * self.noise_variance
        return image + noise
    ...
    def __getitem__(self, item):
        ...
        return noisy_image, torch.tensor(label)
    
# need to add a function in localbase class here 
class LocalBase():
    def  __init__(self,args,train_dataset,test_dataset,client_id):
        noise_variance = 0.01 if client_id == 1 else 0.02 # these r examples for two clients
        self.args = args
        self.client_id = client_id
        self.trainDataset=CreateDataset(train_dataset, args.train_distributed_data[client_id], noise_variance)
        self.testDataset=CreateDataset(test_dataset, args.test_distributed_data[client_id], noise_variance)
